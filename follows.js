(function ( $ ){
	...
	*/
	...
	var defaultKey		= 'eJDJSLZYVXCBvMYbCL488nGpVCwcgXyGTepWbu670vXOp2aK', // Unique master Xively API key to be used as a default
		defaultFeeds	= [988481842], // Comma separated array of Xively Feed ID numbers
		applicationName	= 'Kens Aquaponics', // Replaces Xively logo in the header
			...
			xively.feed.history(id, {  duration: "6hours", interval: 30 }, function (data) {
				if(data.id == id) {
					...
					$('#feed-' + data.id + ' .duration-hour').click(function() {
						...
						updateFeeds(data.id, thisFeedDatastreams, '1hours', 0);
						return false;
					});
